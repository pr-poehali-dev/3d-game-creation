...
interface Game3DWorkingProps {
  character: Character;
}

function Player({ color, onPositionChange }: any) {
  ...
  useFrame(() => {
    ...
    if (keys.current.has('w') || keys.current.has('ц') || keys.current.has('arrowup')) dz -= speed;
    if (keys.current.has('s') || keys.current.has('ы') || keys.current.has('arrowdown')) dz += speed;
    if (keys.current.has('a') || keys.current.has('ф') || keys.current.has('arrowleft')) dx -= speed;
    if (keys.current.has('d') || keys.current.has('в') || keys.current.has('arrowright')) dx += speed;
...
function Scene() {
  return (
    <>
      ...
      {/* Trees */}
      {[[-8, -8], [8, -8], [-8, 8], [8, 8], [-12, 0], [12, 0]].map(([x, z], i) => (
      ...
      <mesh castShadow position={[-15, 2, -15]}>
        ...
        <meshStandardMaterial color="#3D2E5C" />
      </mesh>
...
const Game3DWorking = ({ character }: Game3DWorkingProps) => {
    ...
    <div className="relative w-full h-screen bg-dark-bg">
      <Suspense fallback={
        <div className="w-full h-full flex items-center justify-center">
          <div className="text-primary text-xl">Загрузка 3D мира...</div>
        </div>
      }>
        <Canvas shadows camera={{ position: [0, 8, 10], fov: 60 }}>
          <Scene />
          <Player color={getColor()} onPositionChange={setPlayerPosition} />
